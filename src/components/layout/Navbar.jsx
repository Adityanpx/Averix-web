...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="max-w-container mx-auto px-6 md:px-16 lg:px-24 py-4 flex items-center justify-between">
        {/* Logo */}
          ...
          >
            {navLinks.map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className="text-dark font-medium text-base py-2 border-b border-soft"
            ...
            <Link
              href="/contact"
              className="bg-indigo text-white px-6 py-3 rounded-full text-sm font-semibold text-center mt-2"
            >
              Let's Talk →
            </Link>
          </motion.div>
